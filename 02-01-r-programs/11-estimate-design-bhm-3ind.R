...
if (.Platform$OS.type == "windows") { node.idx = 255 }
...
sp  = read.csv(file="bhm-simulation-design-inputs-3ind.csv");
...
for (s in (1:nsp)) {
 ...
 x=data.frame(estBHMDesign(dType,samp.prior,npg,eqm,gamma0,tau0,alpha0,beta0,nSIM,nMC,nBI));
   colnames(x)<-c("alpha1","gamma1","alpha2","gamma2","sigma2","alpha3","gamma3",
                  "gamma","tau","pp1","pp2","pp3","rr1","rr2","rr3","pp12","rr12");
...
name = paste("./estimate-design-bhm-3ind-",formatC(node.idx, width = 4, format = "d", flag = "0"),".csv",sep="")
